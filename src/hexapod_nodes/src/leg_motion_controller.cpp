...
public:
  ...
  : Node("leg_motion_controller", options)
  {
    ...
    try
    {
      ...
      // Define the publisher for reporting the foot_position of the leg to the main movement controller
      ...
      foot_position = forward_kinematics(current_position.joint0, current_position.joint1, current_position.joint2);
      // Publish the calculated position to the leg_n/foot_position topic
      RCLCPP_INFO(this->get_logger(), "Current leg foot_position:\nx_position: %lf\ny_position: %lf\nz_position: %lf\n", foot_position.x_position, foot_position.y_position, foot_position.z_position);
...
private:
  ...
  void execute(const std::shared_ptr<GoalHandleTarget> goal_handle)
  {
    ...
    for (int i = 0; i < num_divisions && rclcpp::ok() && result->success == false; i++) {
      ...
      // First, calculate the next foot_position to put the leg into. Then, perform inverse kinematics to determine the position
      // of all the joints, then publish that on the command topic.
      
      // Determine the target position of the movement
      // Inverse kinematics: 
      target_position = inverse_kinematics(foot_position.x_position + x_distance_per_division, foot_position.y_position + y_distance_per_division, -foot_position.z_position + z_distance_per_division);


      // Log the results
      RCLCPP_INFO(this->get_logger(), "Sending command to leg %li:\njoint0: %lf\njoint1: %lf\njoint2: %lf", this->get_parameter("leg_id").as_int(), target_position.joint0, target_position.joint1, target_position.joint2);

      // Send the movement command to the leg servo controller on the leg_n/target_joint_angles topic
      ...
      // Calculate the position of the leg in the FootPosition format, then send it to the main movement controller
      // Forward kinematics: 
      foot_position = forward_kinematics(current_position.joint0, current_position.joint1, current_position.joint2);
      // Publish the calculated position to the leg_n/foot_position topic
      RCLCPP_INFO(this->get_logger(), "Current leg foot_position:\nx_position: %lf\ny_position: %lf\nz_position: %lf\n", foot_position.x_position, foot_position.y_position, foot_position.z_position);
  ...
  hexapod_interfaces::msg::JointAngles target_position;
  hexapod_interfaces::msg::JointAngles current_position;
